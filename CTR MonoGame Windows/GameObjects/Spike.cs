...
namespace CTR_MonoGame
{
    class Spike : GameObject
    {
        ...
        public Spike(ContentManager content, Mover m, Vector2 position, float rotation, int size)
            : this(content, m, position, rotation, size, false)
        {}

        public Spike(ContentManager content, Mover m, Vector2 position, float rotation, int size, bool bonus)
            : this(content, m, position, rotation)
        {
            Size = size;
            sprite = new SpikeSprite(content, size, false, bonus);
            UpdateBounds();
        }

        public Spike(ContentManager content, Mover m, Vector2 position, float rotation)
        {
            mover = m;
        ...
        {
            float pWidth = (sprite as SpikeSprite).Width / 2f;
