...
namespace CTR_MonoGame
{
    ...
    public class CTRGame : Game
    {
        ...
        // Game resolution that everything expects
        ...
        {
            // Play music on all platforms (not just arcade device)
            if (SdlMixer.Mix_PlayMusic(gameMusic[R.Next(5)], -1) < 0)
            {
                Console.WriteLine(Sdl.SDL_GetError());
            }
        }
        ...
        {
            // Play music on all platforms (not just arcade device)
            if (SdlMixer.Mix_PlayMusic(menuMusic, 1) < 0)
            {
                Console.WriteLine(Sdl.SDL_GetError());
            }
        }
        ...
        {
            // Play music on all platforms (not just arcade device)
            if (SdlMixer.Mix_PlayMusic(victoryMusic, 1) < 0)
            {
                Console.WriteLine(Sdl.SDL_GetError());
            }
        }
            ...
            : base()
        {
            ...
            // Set window to 540x960 (2x the desired 270x480 internal resolution)
            ...
            // Prevent window from being resized or moved by system
        ...
        protected override void Initialize()
        {
            ...
            // Ensure window size is set correctly
            graphics.PreferredBackBufferWidth = 540;
            graphics.PreferredBackBufferHeight = 960;
            graphics.ApplyChanges();
            
            // Center the window on screen using Windows Forms
            ...
            var windowX = (screenWidth - 540) / 2;
            var windowY = (screenHeight - 960) / 2;
            
            // Set window position using Windows Forms (compatible with older MonoGame)
        ...
        protected override void LoadContent()
        {
            ...
            // Initialize GLDrawer for line drawing
            GLDrawer.Initialize(GraphicsDevice, spriteBatch);
        ...
        protected override void Update(GameTime gameTime)
        {
            ...
            if (Input.KeyJustPressed(Microsoft.Xna.Framework.Input.Keys.Escape))
            {
                Input.IOBoard.SetLockout(false);
                Exit();
            }
            ...
            if (Input.IOBoard.Diag || Input.KeyJustPressed(Microsoft.Xna.Framework.Input.Keys.D))
            {
                mode = Mode.Options;
            }

            if (!Util.OnDevice && Input.KeyJustPressed(Microsoft.Xna.Framework.Input.Keys.E))
            {
                mode = Mode.Editor;
            }

            if (Input.IOBoard.Coin1 || (mode == Mode.Attract && Input.KeyJustPressed(Microsoft.Xna.Framework.Input.Keys.Space)))
            {
            ...
            switch (mode)
            {
                ...
                case Mode.Attract:
                    ...
                    if (FCOptions.Credits >= FCOptions.GameCost || (FCOptions.FreePlay && Input.MouseJustClicked()))
                    {
                        FCOptions.Credits -= FCOptions.GameCost;
                ...
                case Mode.Game:
                    if (LEVEL_TEST)
                    {
                        if (Input.KeyJustPressed(Microsoft.Xna.Framework.Input.Keys.Space) || (level.GameOver && level.Victory))
                        {
                            testLevel++;
                            level = new SingleLevel(Input, Content, (testLevel / 25) + 1, (testLevel % 25) + 1);
                        }
                        if (Input.KeyJustPressed(Microsoft.Xna.Framework.Input.Keys.R) || (level.GameOver && !level.Victory))
                        {
                            level = level.Reset();
                        }
                    }
                    else
                    {
                        ...
                        if (Input.KeyJustPressed(Microsoft.Xna.Framework.Input.Keys.R))
                        {
                            level = level.Reset();
                        }
                    }
        ...
        protected override void Draw(GameTime gameTime)
        {
            ...
            switch (mode)
            {
                ...
                case Mode.Attract:
                    ...
                    {
                        font.Draw(spriteBatch, "Tap Screen", new Vector2(540, 1750), TextSprite.Alignment.Center);
                    }
                    else if (FCOptions.SwipeCard)
                    {
